...
export default function UserLoginPage() {
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_ROUTE}/user/login`, form)
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-slate-900">End User Login</h1>
                    <p className="mt-2 text-sm text-slate-500">Check feature availability for your organization.</p>
                </div>
                ...
                <p className="mt-6 text-sm text-slate-500 text-center">Need an account? <Link className="text-blue-600 font-medium" href="/user/signup">Signup</Link></p>
            </div>
        </div>
    )
}
