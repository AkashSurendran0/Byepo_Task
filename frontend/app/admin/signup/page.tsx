...
export default function AdminSignupPage() {
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_ROUTE}/admin/signup`, form)
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-10">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-slate-900">Organization Admin Signup</h1>
                    <p className="mt-2 text-sm text-slate-500">Create an admin account for an existing organization.</p>
                </div>
                ...
                <p className="mt-6 text-sm text-slate-500 text-center">Already have an account? <Link className="text-blue-600 font-medium" href="/admin/login">Login</Link></p>
            </div>
        </div>
    )
}
