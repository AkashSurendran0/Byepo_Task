...
import { ISignupUser } from "../../domain/use-case/auth-usecase";
...
export class SignupUser implements ISignupUser {
    ...
    async execute(role: UserRole, data: {
        ...
        if (!Types.ObjectId.isValid(organizationId)) {
            return { status: 400, body: { message: "Invalid organization" } };
        }

        const organization = await this._organizationRepository.findById(organizationId);
        if (!organization) {
